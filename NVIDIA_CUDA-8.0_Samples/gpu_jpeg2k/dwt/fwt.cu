...
template <class T, unsigned int start, unsigned int end>
__device__
void process53(const int sign, const int approx, const int a, T *pix_neighborhood)
{
#pragma unroll
	for(int i = start; i <= end; i+=2)
	{
...
__device__
void save_part_results(int p_offset_y, T *results, T *pix_neighborhood)
{
#pragma unroll
	for(int i = 0; i < n; i++)
	{
...
__device__
void save_to_shared(float k, short2 tid, short2 p_offset, int p_size_x, T *results, T shared[][MEMSIZE + 1])
{
#pragma unroll
	for(int i = 0; i < n; i++)
	{
		if(p_offset.y == i)
		{
			shared[tid.x][tid.y] = results[2*i] / k;
			if(tid.x + p_offset.x < p_size_x)// p_size_y
				shared[tid.x + p_offset.x][tid.y] = k * results[2*i + 1];
		}
	}
}
...
template <class T, unsigned int n>
__device__
void save_to_shared_todo(float k, short2 tid, short2 p_offset, int p_size_x, T *results, T shared[][MEMSIZE + 1])
{
#pragma unroll
	for(int i = 0; i < n; i++)
	{
 ...
 */
__device__
void fprocess_53_2(short tidy, const short tidx2, short p_offset_y, int *pix_neighborhood, const int shared[][MEMSIZE + 1],
		int *results)
{
	// Read necessary data
#pragma unroll
...
//	process53<int, 1, 3> (-1, 0, 1, pix_neighborhood);
	...
	// Update 1
//	process53<int, 2, 2> (1, 2, 2, pix_neighborhood);while (tid.y < p_size.x && tid.x < p_size.y)
...
#pragma unroll
	...
	// Predict 1
	process<int, 1, 3> (p53, pix_neighborhood);
	// Update 1
	process<int, 2, 2> (u53, pix_neighborhood);
...
template<class T>
__device__
void read_data(short2 tid, const int2 bid, const short2 p_size, const int2 img_size, const int step_x, const float *idata,
		T shared[][MEMSIZE + 1], T *results, const int offset)
{
	...
	int2 tmp_tid = make_int2(tid.x, tid.y);
...
//			shared[tid.y][tid.x] = idata[bid.x + p_offset.x + (bid.y + p_offset.y) * step_x];
			//printf("%d %d %d %d %d %d\n", bid.x, bid.y, p_offset.x, p_offset.y, step_x,bid.x + p_offset.x + (bid.y + p_offset.y) * step_x);
			//TODO
			shared[tid.x][tid.y] = idata[bid.x + p_offset.x + (bid.y + p_offset.y) * step_x];
...
template<class T>
__device__
void save_data(short2 tid, short2 p_offset, const short2 p_size, const int2 img_size, const int step_x, float *odata,
		T shared[][MEMSIZE + 1])
{
	tid.x = threadIdx.x;
	tid.y = threadIdx.y;
...
void fwt97(const float *idata, float *odata, const int2 img_size, const int2 step)
{
	...
	read_data<float>(tid, bid, p_size, img_size, step.x, idata, shared, results, OFFSET_97);
...
//	while (tid.y < p_size.y + 2 * OFFSET_97 && tidx2 < p_size.x)
	//TODO
	while (tid.y < p_size.x + 2 * OFFSET_97 && tidx2 < p_size.y)
	{
...
//	while (tid.y < p_size.y + 2 * OFFSET_97 && tid.x < p_size.x)
	//TODO
	while (tid.y < p_size.x + 2 * OFFSET_97 && tid.x < p_size.y)
	{
		// Can not dynamically index registers, avoid local memory usage.
...
//		save_to_shared<float, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (k, tid, p_offset, p_size.x, results, shared);
		//TODO
		save_to_shared<float, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (k, tid, p_offset, p_size.y, results, shared);
...
//	while (tid.y < p_size.x && tidx2 < p_size.y)
	//TODO
	while (tid.y < p_size.y && tidx2 < p_size.x)
	{
		fprocess_97(tid.y, tidx2, p_offset.y, pix_neighborhood, shared, results);
...
//	p_offset.x = (int) ceilf(p_size.y / 2.0f);
	...
	// Safe results and rotate
//	while (tid.y < p_size.x && tid.x < p_size.y)
	//TODO
...
//		save_to_shared<float, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (k, tid, p_offset, p_size.y, results, shared);
		//TODO
		save_to_shared_todo<float, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (k, tid, p_offset, p_size.x, results, shared);
	...
	save_data<float>(tid, p_offset, p_size, img_size, step.x, odata, shared);
}
 ...
 */
__global__
void fwt53(const float *idata, float *odata, const int2 img_size, const int2 step)
{
	/* Shared memory for part of the signal */
	...
	read_data<int>(tid, bid, p_size, img_size, step.x, idata, shared, results, OFFSET_53);
	__syncthreads();
	...
	// Process rows
//	while (tid.y < p_size.y + 2 * OFFSET_53 && tidx2 < p_size.x)
	//TODO // Process columns
	while (tid.y < p_size.x + 2 * OFFSET_53 && tidx2 < p_size.y)
	{
//		fprocess_53(tid.y, tidx2, p_offset.y, pix_neighborhood, shared, results);
		fprocess_53_2(tid.y, tidx2, p_offset.y, pix_neighborhood, shared, results);
...
//	p_offset.x = (int) ceilf(p_size.x / 2.0f);
	...
	// safe results and rotate
//	while (tid.y < p_size.y + 2 * OFFSET_53 && tid.x < p_size.x)
	//TODO
	while (tid.y < p_size.x + 2 * OFFSET_53 && tid.x < p_size.y)
	{
		// Can not dynamically index registers, avoid local memory usage.
		//		shared[tid.x][tid.y] = k2 * results[0 + p_offset.y * 2];
		//		if(tid.x + BLOCKSIZEX < p_size.y)
		//			shared[tid.x + BLOCKSIZEX][tid.y] = k1 * results[1 + p_offset.y * 2];
//		save_to_shared<int, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (1, tid, p_offset, p_size.x, results, shared);
		//TODO
		save_to_shared<int, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (1, tid, p_offset, p_size.y, results, shared);
...
//		save_to_shared<int, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (1, tid, p_offset, p_size.y, results, shared);
		//TODO
		save_to_shared_todo<int, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY> (1, tid, p_offset, p_size.x, results, shared);
	...
	save_data<int>(tid, p_offset, p_size, img_size, step.x, odata, shared);
}
