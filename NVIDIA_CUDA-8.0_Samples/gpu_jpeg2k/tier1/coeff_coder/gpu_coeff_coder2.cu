...
__device__ void down(CodeBlockAdditionalInfo &info, CtxWindow &window, CoefficientState *coeffs)
{
  ...
  //  window.tr = 0;
     //printf("w.c:%x ",coeffs[window.pos + 1 - info.width]);
...
__device__ void up(CtxWindow &window, CoefficientState *coeffs)
{
	coeffs[window.pos - 1] = window.l;
     //   printf("the coeffs is %x ",coeffs[window.pos - 1]);
}
...
{
       //  #ifndef debug_aa 
        // printf("the sig is %x %x ",(sig >> (i * 3)) & 0xAA,((sign >> (i * 3)) & 0xAA) >> 1);
      //  #endif
	return signcxlut[
			((sig >> (i * 3)) & 0xAA) |
			(((sign >> (i * 3)) & 0xAA) >> 1)
...
public:
	__device__ char operator()(CtxWindow &window, MQDecoder &dec)
	{
		...
		else
		{
			...
			rest |= mqDecode(dec, CX_UNI) & 1;
		//	window.c |= 1 << (3 * rest);
                     //   printf("the window.c value is %x  ",window.c);
		}
...
public:
	__device__ void operator()(const CodeBlockAdditionalInfo &info, CtxWindow &window, MQCoderStateType &mq, float *sum_dist, unsigned char bitplane)
	{
		...
		CtxReg sig = buildCtxReg(window, 1); // significance context
                    //    #ifndef debug_aa
             //   printf("the sig is %x ",sig);
             //       #endif
               // printf("wc:%x ",window.c);
		rest = -1;
		...
		for(int k = 0; k < 4; k++)
		{
			...
			{
				if(rest >0)//this is the place where be corrected
					rest--;
				else
					SigCodingFunctor()(window, sig, mq, k, info.subband);
			             //   printf("wc forth bit:%d %x ",k,(window.c >> (3 * k)) & 1);
...
__device__ void initCoeffs(const CodeBlockAdditionalInfo &info, CoefficientState *coeffs,int threadId)
{
	unsigned char signOffset = sizeof(int) * 8 - 1;
       // FILE *fp=fopen("./the_analysic_data/initcoeff","w+");
			...
			for(int k = 0; k < 4; k++)
				if(4 * j + k < info.height)
				{
					...
					//Cstates[l++] = (4 * j + k) * info.nominalWidth + i;
                               //  #ifndef debug_aa
                                // printf("c:%002d %002d %002d %002d %006x   ",j,k,i,info.nominalWidth,info.coefficients[(4 * j + k) * info.nominalWidth + i]);
        //#endif
...
__device__ void initDecodingCoeffs(const CodeBlockAdditionalInfo &info, CoefficientState *coeffs,int threadId,int maxThreadBufforLength)
{
		for(int i = 0; i < info.width; i++)
           // for(int i = 0; i < 1; i++)
		for(int j = 0; j < info.stripeNo; j++)
...
__device__ void fillSigns(const CodeBlockAdditionalInfo &info, CoefficientState *coeffs)
{
	unsigned char signOffset = sizeof(int) * 8 - 1;
		...
		//for(int j = 0; j < 1; j++)
		{
			...
			for(int k = 0; k < 4; k++)
				if(((st >> (14 + 3 * k)) & 1) == 0)
                    ...
                    //   #ifndef debug_aa 
             //  printf("the offset:%d %x %x",bitplane,info.coefficients[(4 * j + k) * info.nominalWidth + i],st);
      //  #endif
                           }
			coeffs[j * info.width + i] = st;
                        //printf("convert:%x ",st);
...
{
       //   printf("the uploadmaps is begin\n ");
	for(int i = 0; i < info.width; i++)
            // for(int i = 0; i < 1; i++)
		for(int j = 0; j < info.stripeNo; j++)
		//for(int j = 0; j < 1; j++)               
...
__device__ void encode(CoefficientState *coeffs, byte *out, CodeBlockAdditionalInfo &info, MQEncoder *states, int thread,PcrdCodeblock *pcrdCodeblock = NULL)
{
...
/* #ifndef debug_aa 
		...
		{
			CoefficientState st = coeffs[j * info.width + i];
                        printf("the value is %x\n",coeffs[j * info.width + i]);

                        //printf("convert:%x ",st);
		} 
		printf("bitplane:%d tid:%d info.significantBitsL:%d\n", leastSignificantBP + info.significantBits - 1, blockIdx.x * blockDim.x + threadIdx.x, info.significantBits);
        #endif*/
	...
	//#pragma unroll
		...
		PostCodingFunctor()(mqenc, states, sid, sum_dist, pcrdCodeblock);
          ...
          //  printf("the mqFlush outbuf is %x %x\n",*(coeffs+info.significantBits),*(out+info.significantBits));
	}
}

__device__ void decode(CoefficientState *coeffs, CodeBlockAdditionalInfo &info, byte *in,int threadId,int maxThreadBufforLength)
{
unsigned char leastSignificantBP = 31 - info.magbits;

  __syncthreads();
	MQDecoder mqdec;
	mqInitDec(mqdec, in, info.length,threadId,maxThreadBufforLength);
	...
	if(info.significantBits > 0)
	{
		...
		initDecodingCoeffs(info, coeffs,threadId,maxThreadBufforLength);
		...
		for(unsigned char i = 1; i < info.significantBits; i++)
		{
			...
			uploadMags(info, coeffs, 30 - info.magbits + info.significantBits-i);
               // printf("the finnal outbuf is %d %d\n",*(coeffs+i),*(in+i));
...
{
	int threadId = threadIdx.y*gridDim.x*blockDim.x+blockIdx.x * blockDim.x + threadIdx.x;
     //   printf("%d %d %d %d\n", blockIdx.y,gridDim.x,blockDim.x,blockIdx.x);
	...
	info.length = 0;
	encode<PCRD_EmptyFunctor, CollectMQStatesFunctor>(coeffBuffors + info.magconOffset, outbuf + threadId * maxThreadBufforLength, info, mqstates + threadId,threadId);
                  //      printf("the value of out is %x\n",*(outbuf+8));
...
__global__ void g_encode_pcrd(CoefficientState *coeffBuffors, byte *outbuf, int maxThreadBufforLength, CodeBlockAdditionalInfo *infos, int codeBlocks, MQEncoder *mqstates, int maxStatesPerCodeblock, PcrdCodeblock *pcrdCodeblocks)
{
	...
	info.length = 0;
	encode<PCRD_CollectMQStatesFunctor, PCRD_EmptyFunctor>(coeffBuffors + info.magconOffset, outbuf + threadId * maxThreadBufforLength, info, mqstates + threadId * maxStatesPerCodeblock,threadId, pcrdCodeblocks + threadId * maxStatesPerCodeblock);
...
__global__ void g_decode(CoefficientState *coeffBuffors, byte *inbuf, int maxThreadBufforLength, CodeBlockAdditionalInfo *infos, int codeBlocks)
{
	...
	decode(coeffBuffors + info.magconOffset, info, inbuf + threadId * maxThreadBufforLength,threadId,maxThreadBufforLength);
  __syncthreads();
}
...
//        cudaFuncSetCacheConfig(g_encode, cudaFuncCachePreferL1);
  ...
  //printf("the first value  is %x",*coeffBuffors);
