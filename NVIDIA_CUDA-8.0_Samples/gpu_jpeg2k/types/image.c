...
void init_resolution_lvls(type_tile_comp *tile_comp) {
	...
	for (i = 0; i < tile_comp->num_rlvls; i++) {
		...
		//printf("n=%d , parent_tile tlx=%d , tly=%d , blx=%d , bry=%d \n",n,parent_tile->tlx,parent_tile->tly,parent_tile->brx,parent_tile->bry);
		...
		//printf("res_lvl no:%d num_subbands:%d,dec_lvl_no:%d,tlx:%d,tly:%d,brx:%d,bry:%d,width:%d,height:%d,num_vprc:%d,num_hprc:%d,num_prcs:%d\n",\
				res_lvl->res_lvl_no,res_lvl->num_subbands,res_lvl->dec_lvl_no,res_lvl->tlx,res_lvl->tly,res_lvl->brx,res_lvl->bry,res_lvl->width,res_lvl->height,\
				res_lvl->num_vprc,res_lvl->num_hprc,res_lvl->num_prcs);
...
void init_tile_comps(type_tile *tile, type_parameters *param) {
	...
	for (i = 0; i < parent_img->num_components; i++) {
		...
		//printf("tile_comp no:%d w:%d h:%d,cblk_w:%d,cblk_h:%d\n", i,tile_comp->width, tile_comp->height,tile_comp->cblk_w,tile_comp->cblk_h);
...
void init_tiles(type_image **_img, type_parameters *param) {
	...
	for (i = 0; i < img->num_tiles; i++) {
		...
		tile->parent_img = img;
		//printf("tile no:%d ,tlx:%d,tly:%d,brx:%d,bry:%d,width:%d,height:%d\n",\
						tile->tile_no,tile->tlx,tile->tly,tile->brx,tile->bry,tile->width,tile->height);
...
void set_coding_parameters(type_image *img, type_parameters *param) {
	...
	//printf("image code param{base_step=%d,arget_size=%d\n",	img->coding_param->base_step,img->coding_param->target_size);
}
