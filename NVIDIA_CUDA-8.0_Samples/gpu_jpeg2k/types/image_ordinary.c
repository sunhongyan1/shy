...
void init_image(FIBITMAP* dib, type_image *container, type_parameters *param)
{
	...
	//printf("image {heigh=%d,width=%d,depth=%d，num_components=%d,num_range_bits=%d}\n",\
...
{
//FILE *fp=fopen("./the_analysic_data/the_original_data","w+");
	//	println_start(INFO);
#ifdef READ_TIME
	long int start_load;
	...
	//printf("container{in_file:%s,in_hfile:%s,mct:%d,width:%d,height:%d,depth:%d,sign:%d,num_dlvls:%d,wavelet_type:%d,tile_w:%d,tile_h:%dnum_tiles%d}\n",\
			container->in_file,container->in_hfile,container->mct_compression_method,container->width,container->height,container->depth,\
			container->sign,container->num_dlvls,container->wavelet_type,container->tile_w,container->tile_h,container->num_tiles);
#ifdef READ_TIME
	...
	if(image_type == FIT_BITMAP)
	{
...
#endif
		//printf("FreeImage_Load  dib=%p\n",dib);
		dib = FreeImage_ConvertTo24Bits(dib);
		//printf("FreeImage_ConvertTo24Bits  dib=%p\n",dib);
		...
		//printf("scan_width:%d , mem_size:%d \n",scan_width,mem_size);
...
fprintf(fp,"the show of data unfwt is begin\n");
		{
			...
			cuda_memcpy_dth(tile_comp->img_data_d,pHost,tile->width*tile->height*sizeof(type_data));
			cudaThreadSynchronize();
...
int save_img_ord(type_image *img, const char *filename)
{
	...
	type_tile *tile;
	type_tile_comp *tile_comp;
//FILE *fp=fopen("./the_analysic_data/the_save_data","w+");
