...
void encode_packet_body(type_buffer *buffer, type_res_lvl *res_lvl)
{
	...
	type_subband *sb;
	type_codeblock *cblk;
   // FILE* fp=fopen("number4","wb");
//fprintf(fp,"the encode codestream is begin\n");
	for (i = 0; i < res_lvl->num_subbands; i++) {
		sb = &(res_lvl->subbands[i]);
		for (j = 0; j < sb->num_cblks; j++) {
			cblk = &(sb->cblks[j]);
...
//			println_var(INFO, "%d) %d", cblk->cblk_no, cblk->length);
			sum_size += cblk->length;             
                      //  cblk->codestream=NULL;        
			write_array(buffer, cblk->codestream, cblk->length);
//fwrite(cblk->codestream,sizeof(uint8_t),cblk->length,fp);
//fseek(fp,0,SEEK_SET);
//fflush(fp);
		}
	}
//fprintf(fp,"the  encode codestream is end\n");
	//	println_end(INFO);
//fclose(fp);
}
...
void decode_packet_body(type_buffer *buffer, type_res_lvl *res_lvl)
{
	...
	type_subband *sb;
	type_codeblock *cblk;
      //  FILE* fp=fopen("./the_analysic_data/the_decoder_stream","w+");
//fprintf(fp,"the decode codestream is begin\n");
	for (i = 0; i < res_lvl->num_subbands; i++) {
		sb = &(res_lvl->subbands[i]);
		for (j = 0; j < sb->num_cblks; j++) {
			cblk = &(sb->cblks[j]);
...
//printf("sdfsf\n");
			cuda_h_allocate_mem((void **) &(cblk->codestream), cblk->length);
                       // printf("%d ",buffer1->bp);
                    //  cblk->codestream=buffer1->bp;
			cuda_memcpy_hth(buffer->bp, cblk->codestream, cblk->length);

//
			/*for(int z = 0; z < cblk->length; z++)
			{
				fprintf(fp,"%d", cblk->codestream[z]);
			}
			fprintf(fp,"\n");*/
...
//printf("%d ", cblk->codestream);
//fprintf(fp,"the decode codestream is end\n");
//fclose(fp);
}
/**
 * @brief Encodes tiles.
...
void encode_tiles(type_buffer *buffer, type_tile *tile, int index)
{
	...
	psot_pos = encode_tile_header(buffer, index);
       // FILE* fp=fopen("./the_analysic_data/the_encoder_stream","w+");
	...
	/* One precinct for resolution level and one layer for codestream. */
              // fprintf(fp,"img->num_dlvls is %d \n",img->num_dlvls + 1);
	for (res_no = 0; res_no < img->num_dlvls + 1; res_no++) {
             //  fprintf(fp,"the img->num_dlvls[%d]'s  img->num_components is %d\n",res_no,img->num_components);
		for (comp_no = 0; comp_no < img->num_components; comp_no++) {
 
 //fprintf(fp,"the img->num_dlvls[%d]  img->num_components[%d]'s res_lvl->num_subbands is %d\n",res_no,comp_no,res_lvl->num_subbands);
			...
			encode_packet_body(buffer, res_lvl);
                      // printf("the res-lvl is %d",res_lvl->num_subbands);

//fprintf(fp,"the encode codestream is begin\n");
	/*for (i = 0; i <res_lvl->num_subbands; i++) {
		sb = &(res_lvl->subbands[i]);
 //fprintf(fp,"the img->num_dlvls[%d]  img->num_components[%d] res_lvl->num_subbands[%d]'s sb->cblks is %d\n",res_no,comp_no,i,sb->num_cblks);
		for (int j = 0; j < sb->num_cblks; j++) {
// fprintf(fp,"the cblks_size is %d\n",j);
			cblk = &(sb->cblks[j]);
			/*if(!cblk->num_segments)
			{
				cblk->num_segments++;
...
//
			/*for(int z = 0; z < cblk->length; z++)
			{
				fprintf(fp,"%.2x ", cblk->codestream[z]);
			}
			fprintf(fp,"\n");*/
...
void decode_tiles(type_buffer *buffer, type_tile *tile)
{
    ...
    //  FILE* fp1=fopen("./the_analysic_data/the_decoder_stream","wr");
 // fprintf(fp1,"img->num_dlvls is %d \n",img->num_dlvls+1);
	/* Currently we only support resolution level - layer - component - position progression */
	/* One precinct for resolution level and one layer for codestream. */
	for (res_no = 0; res_no < img->num_dlvls + 1; res_no++) {
          // fprintf(fp1,"the img->num_dlvls[%d]'s  img->num_components is %d\n",res_no,img->num_components);
		for (comp_no = 0; comp_no < img->num_components; comp_no++) {
                                
                                 // if((res_no==0)&&(comp_no==0))
                                 //  res_lvl->num_subbands=res_lvl->num_subbands+3;
 //fprintf(fp1,"the img->num_dlvls[%d]  img->num_components[%d]'s res_lvl->num_subbands is %d\n",res_no,comp_no,res_lvl->num_subbands);
			tile_comp = &(tile->tile_comp[comp_no]);
			res_lvl = &(tile_comp->res_lvls[res_no]);
...
//fprintf(fp1,"the decode codestream is begin\n");
	/*for (i = 0; i <res_lvl->num_subbands; i++) {
		sb = &(res_lvl->subbands[i]);
 //fprintf(fp1,"the img->num_dlvls[%d]  img->num_components[%d] res_lvl->num_subbands[%d]'s sb->cblks is %d\n",res_no,comp_no,i,sb->num_cblks);
		for (int j = 0; j < sb->num_cblks; j++) {
 //fprintf(fp1,"the cblks_size is %d\n",j);
			cblk = &(sb->cblks[j]);
			/*if(!cblk->num_segments)
			{
				cblk->num_segments++;
...
//
			for(int z = 0; z < cblk->length; z++)
			{
				fprintf(fp1,"%.2x ", cblk->codestream[z]);
			}
			fprintf(fp1,"\n");
