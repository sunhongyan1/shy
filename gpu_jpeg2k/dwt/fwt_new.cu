...
#pragma unroll
	for(int i = 0; i < n; i++)
	{
		if(p_offset_y == i)
		{
  if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))  
                        printf("i:%d pix_neighborhood[%d]:%d pix_neighborhood[%d]:%d \n",i,even,pix_neighborhood[even],odd,pix_neighborhood[odd]);
...
#pragma unroll
	for(int i = 0; i < n; i++)
	{
		if(p_offset.y == i)
		{
          ...
          if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
                    printf("i:%d tidy:%d tidx:%d results:%d \n",i,tid.y,tid.x,results[2*i]);
			if(tid.x + p_offset.x < p_size_x)// p_size_y
				{shared[tid.y][tid.x + p_offset.x] = k * results[2*i + 1];
       if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
                    printf("i:%d tidy:%d tidx:%d results:%d \n",i,tid.y,tid.x+p_offset.x,results[2*i+1]);}
		}
	}
}
...
#pragma unroll
 ...
 if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
                    printf("i:%d tidy:%d tidx:%d results:%d \n",i,tid.y+p_offset.x,tid.x,results[2*i+1]);}
		}
	}
}
...
//	process53<int, 2, 2> (1, 2, 2, pix_neighborhood);
	pix_neighborhood[2] += ((pix_neighborhood[1] + pix_neighborhood[3] + 2) >> 2);
                        printf("pix:%d %d %d %d %d ",pix_neighborhood[0],pix_neighborhood[1],pix_neighborhood[2],pix_neighborhood[3],pix_neighborhood[4]);    
	// Can not dynamically index registers, avoid local memory usage.
	//		results[0 + p_offset_y * 2] = pix_neighborhood[4];
	//		results[1 + p_offset_y * 2] = pix_neighborhood[5];
...
//			// If next to last block in column, compute bottom offset to symmetric extension.
			...
			shared[tid.y][tid.x] = idata[bid.x + p_offset.x + (bid.y + p_offset.y) * step_x];
              if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==2&&(tid.x)==0)||(tid.y==3&&(tid.x)==0)||(tid.y==4&&(tid.x)==0)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18)||(tid.y==0&&(tid.x)==1)||(tid.x==2&&(tid.y)==0)||(tid.x==3&&(tid.y)==0)||(tid.x==4&&(tid.y)==0)||(tid.x==1&&(tid.y)==16)||(tid.x==1&&(tid.y)==18))
                        printf("%d,%d,idata[%d]:%d ",tid.y,tid.x,bid.x + p_offset.x + (bid.y + p_offset.y) * step_x,shared[tid.y][tid.x]);
                     //   if()
			tid.x += BLOCKSIZEX;
		}
		tid.x = threadIdx.x;
		...
		T shared[][MEMSIZE + 1])
{
	...
	while (tid.y < p_offset.y)
	{
		...
		while (tid.x < p_offset.x)
		{
			odata[tid.x + blockIdx.x * PATCHX_DIV_2 + (tid.y + blockIdx.y * PATCHY_DIV_2) * step_x] = shared[tid.y][tid.x];
                      if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
           //if((tid.y==0&&(tid.x)==0))
 printf("%d,%d:odata[%d]:%d ",tid.y,tid.x,tid.x + blockIdx.x * PATCHX_DIV_2 + (tid.y + blockIdx.y * PATCHY_DIV_2) * step_x,shared[tid.y][tid.x]);
   ...
   odata[tid.x + (int) ceilf(img_size.x / 2.0f) + blockIdx.x * PATCHX_DIV_2 + (tid.y + blockIdx.y * PATCHY_DIV_2) * step_x]
      ...
      if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
                        printf("%d,%d:odata[%d]:%d ",tid.y,tid.x + p_offset.x,tid.x + (int) ceilf(img_size.x / 2.0f) + blockIdx.x * PATCHX_DIV_2 + (tid.y + blockIdx.y * PATCHY_DIV_2) * step_x,shared[tid.y][tid.x + p_offset.x]);
}
 ...
 odata[tid.x + blockIdx.x * PATCHX_DIV_2 + (tid.y + (int) ceilf(img_size.y / 2.0f) + blockIdx.y * PATCHY_DIV_2) * step_x]
                       ...
                       //if((tid.y==0&&tid.x==0)||(tid.y==0&&tid.x==1)||(tid.y==0&&tid.x==2)||(tid.y==0&&tid.x==3))
        if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
  printf("%d,%d:odata[%d]:%d ",tid.y + p_offset.y,tid.x,tid.x + blockIdx.x * PATCHX_DIV_2 + (tid.y + (int) ceilf(img_size.y / 2.0f) + blockIdx.y * PATCHY_DIV_2) * step_x,shared[tid.y + p_offset.y][tid.x]);
}
					...
					+ blockIdx.y * PATCHY_DIV_2 < img_size.y)
{
				odata[tid.x + (int) ceilf(img_size.x / 2.0f) + blockIdx.x * PATCHX_DIV_2 + (tid.y + (int) ceilf(img_size.y / 2.0f)
                     ...
                     //  if((tid.y==0&&tid.x==0)||(tid.y==0&&tid.x==1)||(tid.y==0&&tid.x==2)||(tid.y==0&&tid.x==3))
               if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
  printf("%d,%d:odata[%d]:%d ",tid.y + p_offset.y,tid.x + p_offset.x,tid.x + (int) ceilf(img_size.x / 2.0f) + blockIdx.x * PATCHX_DIV_2 + (tid.y + (int) ceilf(img_size.y / 2.0f)+ blockIdx.y * PATCHY_DIV_2) * step_x,shared[tid.y + p_offset.y][tid.x + p_offset.x]);
}
			tid.x += BLOCKSIZEX;
		}
		tid.x = threadIdx.x;
...
void fwt53_new(const float *idata, float *odata, const int2 img_size, const int2 step)
{
	...
	while (tid.y * 2 < p_size.y && tid.x < p_size.x + 2 * OFFSET_53)
	{
		...
		for (int i = 0; i < 5; i++)
		{
           ...
           if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
                  printf("i:%d shared[%d][%d]:%d ",i,tid.y * 2 + i - 2 + OFFSET_53,tid.x,shared[tid.y * 2 + i - 2 + OFFSET_53][tid.x]);
		}

		pix_neighborhood_y[1] = pix_neighborhood[1]-floorf((pix_neighborhood[0] + pix_neighborhood[2])/2);
		pix_neighborhood_y[3] = pix_neighborhood[3]-floorf((pix_neighborhood[2] + pix_neighborhood[4])/2);
		// Update 1
	//	process53<int, 2, 2> (1, 2, 2, pix_neighborhood);
		pix_neighborhood_y[2] =pix_neighborhood[2]+floorf((pix_neighborhood_y[1] + pix_neighborhood_y[3] + 2)/4);
                                    if((tid.y==0&&(tid.x)==0)||(tid.y==1&&(tid.x)==0)||(tid.y==1&&(tid.x)==1)||(tid.y==1&&(tid.x)==2)||(tid.y==1&&(tid.x)==17)||(tid.y==1&&(tid.x)==16)||(tid.y==1&&(tid.x)==18))
                printf("pix[0]:%d pix[1]:%d pix[2]:%d pix[3]:%d pix[4]:%d pix_y[1]:%d pix_y[2]:%d pix_y[3]:%d \n",pix_neighborhood[0],pix_neighborhood[1],pix_neighborhood[2],pix_neighborhood[3],pix_neighborhood[4],pix_neighborhood_y[1],pix_neighborhood_y[2],pix_neighborhood_y[3]);
		save_part_results_new<int, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY, 2, 3> (p_offset.y, results, pix_neighborhood_y);
 if((tid.y==0&&(tid.x)==0))
                  printf("shared[0][1]:%d ",shared[0][1]);
		p_offset.y++;
		tid.x += BLOCKSIZEY;
	}
	__syncthreads();
 ...
 if((tid.y==0&&(tid.x)==0))
	...
	while (tid.y < p_size.y && tid.x * 2 < p_size.x)
	{
       ...
       if((tid.y==0&&(tid.x)==0))
                  printf("i:%d shared[%d][%d]:%d ",i,tid.y,tid.x * 2 + i - 2 + OFFSET_53,shared[tid.y][tid.x * 2 + i - 2 + OFFSET_53]);
		}
	...
	//	process53<int, 2, 2> (1, 2, 2, pix_neighborhood);
		pix_neighborhood_y[2] =pix_neighborhood[2]+floorf((pix_neighborhood_y[1] + pix_neighborhood_y[3] + 2)/4);
               if((tid.y==0&&(tid.x)==0))
                printf("pix[0]:%d pix[1]:%d pix[2]:%d pix[3]:%d pix_y[1]:%d pix_y[2]:%d pix_y[3]:%d \n",pix_neighborhood[0],pix_neighborhood[1],pix_neighborhood[2],pix_neighborhood[3],pix_neighborhood_y[1],pix_neighborhood_y[2],pix_neighborhood_y[3]);
		save_part_results_new<int, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY, 2, 3> (p_offset.y, results, pix_neighborhood_y);
