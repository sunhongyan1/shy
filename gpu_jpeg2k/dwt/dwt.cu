...
//	println_start(INFO);
	...
	type_image *img = tile->parent_img;
       // FILE *fp=fopen("./the_analysic_data/the_fwted_data","w+");
...
/*fflush(fp);
printf("the show of data unfwt is begin\n");
fprintf(fp,"the show of data unfwt is begin\n");
		{
			int xxx = 0;
			type_data *pHost;
			(pHost) = (float *)malloc(tile->width*tile->height*sizeof(type_data));
			cudaMemcpy(pHost,tile_comp->img_data_d,tile->width*tile->height*sizeof(type_data),cudaMemcpyDeviceToHost);
			cudaThreadSynchronize();
			for(xxx=0;xxx<tile->width*tile->height;xxx++)
			{
				//printf("%8.3f,", pHost[xxx]);
                          fprintf(fp,"%-8.3f ",pHost[xxx]);
				if(xxx%16 == 15)
				{
					//printf("\n");
                                        fprintf(fp,"\n");
			...
			int iend = tile->width*tile->height-128;
			//printf("iend = %d\n",iend);
			for(xxx=0;xxx<128;xxx++)
			{
				//printf("%8.3f,", pHost[iend+xxx]);
                                fprintf(fp,"%-8.3f ",pHost[iend+xxx]);
				if(xxx%16 == 15)
				{
					//printf("\n");
                                           fprintf(fp,"\n");
			...
			free(pHost);
//printf("the show of data unfwt is end\n");
fprintf(fp,"the show of data unfwt is end\n");
		}*/
...
/*fflush(fp);
//printf("the show of data unfwt is begin\n");
fprintf(fp,"the show of data endfwt is begin\n");
		{
			int xxx = 0;
			type_data *pHost;
			(pHost) = (float *)malloc(tile->width*tile->height*sizeof(type_data));
...
//	println_end(INFO);
//fclose(fp);
//printf("the program is running this \n");
}
...
//println_start(INFO);
	...
	type_tile_comp *tile_comp;
	type_image *img = tile->parent_img;
    // FILE *fp=fopen("./the_analysic_data/the_uniwt_data","w+");
...
//printf("erererer\n");
	...
	for(i = 0; i < tile->parent_img->num_components; i++)
	{
     ...
     type_data *h_fdata;
			...
			//printf("the show of data uniwt is begin\n");
//fprintf(fp,"the show of data uniwt is begin\n");
			//cudaMemcpy(pHost,tile_comp->img_data_d,tile->width*tile->height*sizeof(type_data),cudaMemcpyDeviceToHost);
			//cudaThreadSynchronize();
...
fprintf(fp,"\n");
			...
			{
				//printf("%8.3f,", pHost[iend+xxx]);
                                 fprintf(fp,"%-8.3f ",pHost[xxx+iend]);
				if(xxx%16 == 15)
				{
					//printf("\n");
fprintf(fp,"\n");
...
printf("the show of data uniwt is end\n");
fprintf(fp,"the show of data uniwt is end\n");*/
...
#else
                             //  printf("the program is running this3\n");
			tile_comp->img_data_d = iwt_2d(DWT53, tile_comp);
                                  // printf("the program is running this4\n");
#endif

		}
//printf("the show of data endiwt is begin\n");
//fprintf(fp,"the show of data endiwt is begin\n");
			/*cudaMemcpy(pHost,tile_comp->img_data_d,tile->width*tile->height*sizeof(type_data),cudaMemcpyDeviceToHost);
			cudaThreadSynchronize();
...
fprintf(fp,"\n");
			...
			iend = tile->width*tile->height-128;
			//printf("iend = %d\n",iend);
			for(xxx=0;xxx<128;xxx++)
			{
				//printf("%8.3f,", pHost[iend+xxx]);
...
//printf("the show of data uniwt is end\n");
fprintf(fp,"the show of data endiwt is end\n");*/
	}
//free(pHost);
//fclose(fp);
}
