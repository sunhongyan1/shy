...
template <class T, unsigned int start, unsigned int end>
__device__ void process2_new(const int sign, const int a, T *pix_neighborhood, T *pix_neighborhood_y,int round)
{
int i;
...
#pragma unroll
	...
	{
		pix_neighborhood_y[i]=pix_neighborhood[i]+sign * floorf((pix_neighborhood[i-1] + pix_neighborhood[i+1] + round)/(1<<a));
              if(i==3)
               pix_neighborhood_y[i]=pix_neighborhood[i]+sign * floorf((pix_neighborhood_y[i-1] + pix_neighborhood_y[i+1] + round)/(1<<a));
                if((tid.y==0&&tid.x==0))
                printf("pix:%d %d %d pix_y:%d %d %d",pix_neighborhood[i-1],pix_neighborhood[i],pix_neighborhood[i+1],pix_neighborhood_y[i-1],pix_neighborhood_y[i],pix_neighborhood_y[i+1]);
	}
}
...
#pragma unroll
	for(int i = 0; i < n; i++)
	{
		if(p_offset_y == i)
		{
                    if((tid.y==0&&tid.x==0)||(tid.y==0&&tid.x==1)||(tid.y==0&&tid.x==2)||(tid.y==0&&tid.x==3))
                    printf("i:%d ",i);
	...
	for(int i = 0; i < n; i++)
	{
		if(p_offset_y == i)
		{
			shared[even_tid.x][even_tid.y] = k * results[2*i];
                if((tid.y==0&&tid.x==0)||(tid.y==0&&tid.x==1)||(tid.y==0&&tid.x==2)||(tid.y==0&&tid.x==3))
                  printf("i:%d shared[%d][%d]:%d \n",i,even_tid.x,even_tid.x,shared[even_tid.x][even_tid.y]);
			if(if_tid < p_size_x)// p_size_y
			{	shared[odd_tid.x][odd_tid.y] = k * results[2*i+1];
    if((tid.y==0&&tid.x==0)||(tid.y==0&&tid.x==1)||(tid.y==0&&tid.x==2)||(tid.y==0&&tid.x==3))
                  printf("i:%d shared[%d][%d]:%d \n",i,odd_tid.x,odd_tid.y,shared[odd_tid.x][odd_tid.y]);}
		}
	}
}
...
void iprocess_53_new(const short tidx2, short tidy, short p_offset_y, const short p_size_x, const short p_size_y,
		int *pix_neighborhood, int *pix_neighborhood_y,const int shared[][sm_size], int *results)
{
short2 tid = make_short2(threadIdx.x, threadIdx.y);
	// Process rows
	while (tidy < p_size_y && tidx2 < p_size_x)
...
if((tid.y==0&&tid.x==0)||(tid.y==0&&tid.x==1)||(tid.y==0&&tid.x==2)||(tid.y==0&&tid.x==3))
               printf("i:%d shared[%d][%d]:%d \n",i,tidy,tidx2 + i - 2 + OFFSET_53,shared[tidy][tidx2 + i - 2 + OFFSET_53]);
		}
...
/*		// Update 1
 ...
 //if((tid.y==0&&tid.x==0))
               //printf("i:%d shared[%d][%d]:%d \n",i,tidy,tidx2 + i - 2 + OFFSET_53,pix_neighborhood[0]);
		// Update 1
		process2_new<int, 2, 4> (-1, 2, pix_neighborhood,pix_neighborhood_y,2);

		// Predict 1
		process2_new<int, 3, 3> (1, 1, pix_neighborhood,pix_neighborhood_y,0);

		// Can not dynamically index registers, avoid local memory usage.
		//		results[0 + p_offset_y * 2] = pix_neighborhood[4];
		//		results[1 + p_offset_y * 2] = pix_neighborhood[5];
		save_part_results_new<int, (MEMSIZE + (BLOCKSIZEY - 1)) / BLOCKSIZEY, 2, 3> (p_offset_y, results, pix_neighborhood_y);
...
{
                                        //  printf("the program is running this4\n");
	...
	while (tid.y < p_size_y.x + 2 * offset)
	{
		while (tid.x < p_size_x.x + 2 * offset)
		{
			...
			/*p_offset_x.y
					//= ((img_size.x - bidx.y < BLOCKSIZEX + offset) ? ((img_size.x - bidx.y/* + (img_size.x - bidx.y - p_size_x.y) - 2*/
                         //-1) - (tid.x
...
shared[2 * tid.y][2 * tid.x] = k * idata[bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x];
	//__syncthreads();
                  if((tid.y==0&&tid.x==0)||(tid.y==1&&tid.x==0)||(tid.y==2&&tid.x==0)||(tid.y==3&&tid.x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==256))
         printf("shared[%d][%d]:%d %d ",2 * tid.x,2 * tid.y,bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x,shared[2 * tid.x][2 * tid.y]);
// printf("shared[%d][%d]:%d \n",2 * tid.x,2 * tid.y,shared[2 * tid.x][2 * tid.y]);}
//printf("shared[%d][%d]:%d %d %d ",2 * tid.x,2 * tid.y,bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x,idata[bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x],shared[2 * tid.x][2 * tid.y]);
			/* HL - top right part block */
      //shared[2 * tid.x][2 * tid.y] = k * idata[tid.x + bidx.x + (tid.y + bidy.x) * step_x];
			if (bidx.y + p_offset_x.y < img_size.x/* && bidy.x + p_offset_y.x < img_size.y*/)
				{
//shared[2 * tid.x + 1/* + p_size_y.x + OFFSET_97*/][2 * tid.y] = k * idata[tid.x + bidx.y + (bidy.x
						//+ tid.y) * step_x];
shared[2 * tid.y + 1/* + p_size_y.x + OFFSET_97*/][2 * tid.x] = k * idata[bidx.y + p_offset_x.y + (bidy.x
						+ p_offset_y.x) * step_x];
                              if((tid.y==0&&tid.x==0)||(tid.y==1&&tid.x==0)||(tid.y==2&&tid.x==0)||(tid.y==3&&tid.x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==256))
                        printf("shared[%d][%d]:%d %d ",2 * tid.x + 1,2 * tid.y,bidx.y + p_offset_x.y + (bidy.x+ p_offset_y.x) * step_x,shared[2 * tid.x + 1/* + p_size_y.x + OFFSET_97*/][2 * tid.y]);
// printf("shared[%d][%d]:%d %d %d ",2 * tid.x + 1,2 * tid.y,bidx.y + p_offset_x.y + (bidy.x+ p_offset_y.x) * step_x,idata[bidx.y + p_offset_x.y + (bidy.x+p_offset_y.x) * step_x],shared[2 * tid.x + 1/* + p_size_y.x + OFFSET_97*/][2 * tid.y]);
}
			/* LH - bottom left part block */
			if (/*bidx.x + p_offset_x.x < img_size.x && */bidy.y + p_offset_y.y < img_size.y)
				{
  shared[2 * tid.y][2 * tid.x + 1/* + p_size_x.x + OFFSET_97*/] = idata[bidx.x + p_offset_x.x + (bidy.y
						+ p_offset_y.y) * step_x] / k;
                             if((tid.y==0&&tid.x==0)||(tid.y==1&&tid.x==0)||(tid.y==2&&tid.x==0)||(tid.y==3&&tid.x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==256))
                         printf("shared[%d][%d]:%d %d ",2 * tid.x ,2 * tid.y+1,bidx.x + p_offset_x.x + (bidy.y+ p_offset_y.y) * step_x,shared[2 * tid.x][2 * tid.y + 1/* + p_size_x.x + OFFSET_97*/]);
 //printf("shared[%d][%d]:%d %d %d ",2 * tid.x ,2 * tid.y+1,bidx.x + p_offset_x.x + (bidy.y+ p_offset_y.y) * step_x,idata[bidx.x + p_offset_x.x + (bidy.y+p_offset_y.y) * step_x],shared[2 * tid.x][2 * tid.y + 1/* + p_size_x.x + OFFSET_97*/]);
}
			/* HH - bottom right part block */
			if (bidx.y + p_offset_x.y < img_size.x && bidy.y + p_offset_y.y < img_size.y)
				{
           shared[2 * tid.y + 1/* + p_size_y.x + OFFSET_97*/][2 * tid.x + 1/* + p_size_x.x + OFFSET_97*/] = idata[bidx.y + p_offset_x.y + (bidy.y + p_offset_y.y) * step_x] / k;
                           if((tid.y==0&&tid.x==0)||(tid.y==1&&tid.x==0)||(tid.y==2&&tid.x==0)||(tid.y==3&&tid.x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==0)||(bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x==256))
                        printf("shared[%d][%d]:%d %d ",2 * tid.x + 1,2 * tid.y+1,bidx.y + p_offset_x.y + (bidy.y+ p_offset_y.y) * step_x,shared[2 * tid.x + 1/* + p_size_y.x + OFFSET_97*/][2 * tid.y + 1/* + p_size_x.x + OFFSET_97*/]);
 //printf("shared[%d][%d]:%d %d %d ",2 * tid.x + 1,2 * tid.y+1,bidx.y + p_offset_x.y + (bidy.y+ p_offset_y.y) * step_x,idata[bidx.y + p_offset_x.y + (bidy.y + p_offset_y.y) * step_x],shared[2 * tid.x + 1/* + p_size_y.x + OFFSET_97*/][2 * tid.y + 1/* + p_size_x.x + OFFSET_97*/]);
}
...
//			: p_offset_y.y);
			...
			/* LL - top left part block */
			//shared[2 * tid.x][2 * tid.y] = k * idata[bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x];
                       if((tid.y==0&&tid.x==0)||(tid.y==1&&tid.x==0)||(tid.y==2&&tid.x==0)||(tid.y==3&&tid.x==0))
    printf("shared[%d][%d]:%d %d %d ",2 * tid.x,2 * tid.y,bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x,idata[bidx.x + p_offset_x.x + (bidy.x + p_offset_y.x) * step_x],shared[2 * tid.x][2 * tid.y]);
...
if((tid.y==0&&tid.x==0)||(tid.y==0&&tid.x==1)||(tid.y==0&&tid.x==2)||(tid.y==0&&tid.x==3))
                        printf("tid.y:%d tid.x:%d odata[%d]:%d ",tid.y,tid.x,2 * bidx_x + tid.x + (2 * bidy_x + tid.y) * step_x,shared[tid.y][tid.x]);
			tid.x += BLOCKSIZEX;
		}
		tid.x = threadIdx.x;
		tid.y += BLOCKSIZEY;
...
void iwt53_new(const float *idata, float *odata, const int2 img_size, const int2 step)
{
	...
	__shared__ int shared_y[MEMSIZE][MEMSIZE + 1];
...
//	const int2 ll_sub = make_int2((int) ceilf(img_size.x / 2.0), (int) ceilf(img_size.y / 2.0));
	...
	__syncthreads();
// printf("shared[0][2]:%d \n",shared[0][2]);
//read_data<int, MEMSIZE + 1>(1, tid, bidx, bidy, p_size_x, p_size_y, ll_sub, img_size, step.x, idata, shared, OFFSET_53/2);
	// thread x id
	tid.x = threadIdx.x;
	// thread y id
	tid.y = threadIdx.y;
	...
	// Process columns
	iprocess_53_new<MEMSIZE + 1>(tidx2, tid.y, p_offset_y_t, p_size_sum.y, p_size_sum.x + 2 * OFFSET_53, pix_neighborhood,pix_neighborhood_y,shared, results);
	...
	// Process rows
	iprocess_53_new<MEMSIZE + 1>(tidx2, tid.y, p_offset_y_t, p_size_sum.x, p_size_sum.y, pix_neighborhood,pix_neighborhood_y,shared, results);
