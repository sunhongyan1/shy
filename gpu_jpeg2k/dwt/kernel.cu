...
//	println_start(INFO);
	...
	//printf("tile_comp->width=%d , tile_comp->height=%d,img_size x=%d y=%d,step x=%d y=%d\n",\
			tile_comp->width,tile_comp->height,img_size.x,img_size.y,step.x,step.y);
	for (i = 0; i < tile_comp->num_dlvls; i++) {
		...
		switch(filter)
		{
			case DWT97:
				...
				break;
			case DWT53:
				fwt53_new<<<1, dim3(BLOCKSIZEX, BLOCKSIZEY)>>>(d_idata, d_odata, img_size, step);
				break;
		}
...
//		printf("OK gridx %d, ..gridy %d img_size.x %d img_size.y %d\n", grid_size.x, grid_size.y, img_size.x, img_size.y);
		...
		switch(filter)
		{
			case DWT97:
				...
				break;
			case DWT53:
				iwt53_new<<<1, dim3(BLOCKSIZEX, BLOCKSIZEY)>>>(d_idata, d_odata, img_size, step);
				break;
		}
