...
float gpuEncode(EntropyCodingTaskInfo *infos, int count, int targetSize)
{
	...
	{
		//printf("No pcrd\n");
		CHECK_ERRORS(GPU_JPEG2K::launch_encode((int) ceil((float) codeBlocks / (THREADS*THREADS/2)),dim3(THREADS, THREADS/2), d_stBuffors, d_outbuf, maxOutLength, d_infos, codeBlocks));
	}
	else
	{
		//printf("Pcrd\n");
		CHECK_ERRORS(GPU_JPEG2K::launch_encode_pcrd((int) ceil((float) codeBlocks / (THREADS*THREADS/2)), dim3(THREADS, THREADS/2), d_stBuffors, d_outbuf, maxOutLength, d_infos, codeBlocks, targetSize));
	}
...
float gpuDecode(EntropyCodingTaskInfo *infos, int count)
{
	...
	CHECK_ERRORS(GPU_JPEG2K::launch_decode((int) ceil((float) codeBlocks / (THREADS*THREADS/2)),dim3(THREADS, THREADS/2),d_stBuffors, d_inbuf, maxOutLength, d_infos, codeBlocks));
