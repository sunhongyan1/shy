...
//	start_measure();
	...
	for(i = 0; i < img->num_tiles; i++)
	{
		...
		for(j = 0; j < img->num_components; j++)
		{
			...
			dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE);
			int level_shift = img->num_range_bits - 1;
			//printf("level_shift=%d,tile->width=%d,tile->height=%d\n",level_shift,tile->width,tile->height);
			...
			/*{
				...
				cudaMemcpy(pHost,idata,tile->width*tile->height*sizeof(type_data),cudaMemcpyDeviceToHost);
				cudaThreadSynchronize();
